...
import { AlertCircle, Calculator, LoaderCircle, ShieldCheck } from "lucide-react";
...
export function PersistedRiskScreen({ requestId }: { requestId: string }) {
  ...
  useEffect(() => {
    ...
    Promise.all([getRequest(requestId), getPlan(requestId), getSimulation(requestId), getRisk(requestId)])
      .then(([nextRequest, nextPlan, nextSimulation, nextAssessment]) => {
        if (active) { setRequest(nextRequest); setPlan(nextPlan); setSimulation(nextSimulation); setAssessment(nextAssessment); }
      })
  ...
  return <AppShell pageTitle="Risk Engine"><main className="mx-auto max-w-[1500px] px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
    ...
    <section className="surface-panel mt-7 rounded-[12px] border border-white/[0.08] bg-[var(--surface)] p-5"><div className="flex items-start gap-3"><span className="grid size-9 shrink-0 place-items-center rounded-[7px] border border-[rgb(167_139_250/0.24)] text-[var(--intelligence)]"><ShieldCheck size={18} /></span><div><p className="text-xs font-semibold">{assessment.summary}</p><p className="mt-1 text-[10px] text-[var(--text-secondary)]">No AI inference, production access, policy decision, approval, or execution was performed.</p></div></div><div className="mt-5 grid gap-3 border-t border-white/[0.06] pt-4 sm:grid-cols-3"><div><p className="text-[8px] uppercase tracking-wider text-[var(--text-muted)]">Request</p><p className="mt-2 truncate font-mono text-[9px]">{request.id}</p></div><div><p className="text-[8px] uppercase tracking-wider text-[var(--text-muted)]">Plan</p><p className="mt-2 truncate font-mono text-[9px]">{plan.id}</p></div><div><p className="text-[8px] uppercase tracking-wider text-[var(--text-muted)]">Simulation</p><p className="mt-2 truncate font-mono text-[9px]">{simulation.id}</p></div></div></section>
    ...
    <div className="mt-6 flex flex-wrap items-center justify-between gap-3 rounded-[10px] border border-[rgb(167_139_250/0.2)] bg-[rgb(167_139_250/0.04)] p-4"><p className="text-[10px] text-[var(--text-secondary)]">Assessment persisted from synthetic evidence. Governance stages remain pending.</p><Link href={`/shadow-workspace/${requestId}`} className="text-xs font-medium text-[var(--intelligence)]">Review simulation</Link></div>
  </main></AppShell>;
}
