...
import { AlertCircle, ArrowRight, Check, CircleDot, Clock3, Database, FileClock, LoaderCircle, Plus, Server, ShieldAlert } from "lucide-react";
...
function PreviewBoundary() {
  return <div className="mt-4 grid gap-4 lg:grid-cols-[0.8fr_1.2fr]"><section className="surface-panel rounded-[12px] border border-white/[0.08] bg-[var(--surface)] p-4 sm:p-5"><div className="flex items-start gap-3"><ShieldAlert size={16} className="mt-0.5 shrink-0 text-[var(--warning)]" /><div><h2 className="text-sm font-medium">Start with a governed request</h2><p className="mt-2 text-[10px] leading-5 text-[var(--text-secondary)]">Persist a request before generating its deterministic structured action plan. Resource simulation, risk, policy, approvals, and execution remain unavailable.</p></div></div><div className="mt-5 flex items-center gap-3 border-y border-white/[0.06] py-4"><span className="grid size-7 place-items-center rounded-full border border-[rgb(167_139_250/0.25)] text-[var(--intelligence)]"><CircleDot size={13} /></span><div><p className="text-xs text-[var(--text-secondary)]">Submit a request to establish its identity</p><p className="mt-1 font-mono text-[8px] text-[var(--text-muted)]">PERSISTENCE · THEN STRUCTURED PLANNING</p></div></div></section><section className="surface-panel rounded-[12px] border border-white/[0.08] bg-[var(--surface)] p-4 sm:p-5"><p className="text-[9px] font-medium uppercase tracking-[0.15em] text-[var(--warning)]">Phase 1 sample boundary</p><h2 className="mt-2 text-sm font-medium">REQ-8F2A remains preview-only</h2><p className="mt-2 text-[10px] leading-5 text-[var(--text-secondary)]">The legacy sample at the base Action Plans route is isolated from persisted plans. Real plans use request-specific routes and never inherit sample counts or risk.</p><dl className="mt-5 grid gap-px overflow-hidden rounded-[8px] border border-white/[0.06] bg-white/[0.06] sm:grid-cols-3">{[["Sample actions", "3"], ["Sample resources", "143"], ["Sample risk", "HIGH · 72"]].map(([label, value]) => <div key={label} className="bg-[var(--surface)] p-3"><dt className="text-[8px] text-[var(--text-muted)]">{label}</dt><dd className="mt-2 font-mono text-[10px] text-[var(--text-secondary)]">{value}</dd></div>)}</dl></section></div>;
}

function PersistedContext({ request, planning, planningError, onGenerate }: { request: ShadowRequest; planning: boolean; planningError: string; onGenerate: () => void }) {
  const planned = request.status === "PLANNED";
  const failed = request.status === "FAILED";
  return <div className="mt-4 grid gap-4 lg:grid-cols-[1.2fr_0.8fr]"><section className="surface-panel rounded-[12px] border border-[rgb(93_211_158/0.18)] bg-[var(--surface)] p-4 sm:p-5"><div className="flex items-start gap-3"><span className="grid size-9 shrink-0 place-items-center rounded-[8px] bg-[rgb(93_211_158/0.08)] text-[var(--safe)]"><Check size={16} /></span><div><p className="text-xs font-medium text-[var(--safe)]">Request recorded</p><p className="mt-1 text-[10px] text-[var(--text-secondary)]">Persisted successfully as the governed source for structured planning.</p></div></div><dl className="mt-5 grid gap-px overflow-hidden rounded-[8px] border border-white/[0.06] bg-white/[0.06] sm:grid-cols-2"><div className="bg-[var(--surface)] p-3"><dt className="text-[8px] uppercase tracking-wider text-[var(--text-muted)]">Request ID</dt><dd className="mt-2 break-all font-mono text-[9px] text-white">{request.id}</dd></div><div className="bg-[var(--surface)] p-3"><dt className="text-[8px] uppercase tracking-wider text-[var(--text-muted)]">Status</dt><dd className="mt-2 font-mono text-[10px] text-[var(--safe)]">{request.status}</dd></div><div className="bg-[var(--surface)] p-3"><dt className="text-[8px] uppercase tracking-wider text-[var(--text-muted)]">Environment</dt><dd className="mt-2 flex items-center gap-1.5 text-[10px] text-[var(--text-secondary)]"><Server size={11} />{environmentLabel(request.environment)}</dd></div><div className="bg-[var(--surface)] p-3"><dt className="text-[8px] uppercase tracking-wider text-[var(--text-muted)]">Created</dt><dd className="mt-2 flex items-center gap-1.5 text-[10px] text-[var(--text-secondary)]"><Clock3 size={11} />{formatTimestamp(request.createdAt)}</dd></div></dl></section><section className="surface-panel rounded-[12px] border border-white/[0.08] bg-[var(--surface)] p-4 sm:p-5"><p className="text-xs font-medium">Structured planning</p><p className="mt-2 text-[10px] leading-5 text-[var(--text-muted)]">{planned ? "A validated deterministic action plan is persisted for this request." : failed ? "The deterministic planner could not safely understand this request. Create a refined request to try again." : "Generate a deterministic semantic plan. No resource will be simulated or changed."}</p><div className="mt-5 flex items-center gap-3 border-y border-white/[0.06] py-4"><span className="grid size-7 place-items-center rounded-full border border-[rgb(167_139_250/0.25)] text-[var(--intelligence)]">{planning ? <LoaderCircle size={13} className="animate-spin" /> : <CircleDot size={13} />}</span><div><p className="text-xs text-[var(--text-secondary)]">{planning ? "Building structured plan" : planned ? "Action plan ready" : failed ? "Planning unavailable" : "Ready for planning"}</p><p className="mt-1 font-mono text-[8px] text-[var(--text-muted)]">SIMULATION PROVIDER · NO EXTERNAL AI</p></div></div>{planningError && <p role="alert" className="mt-3 text-[10px] leading-5 text-[var(--critical)]">{planningError}</p>}{planned ? <Link href={`/action-plans/${request.id}`} className="mt-4 flex items-center justify-center gap-2 rounded-[7px] bg-white px-3 py-2.5 text-xs font-medium text-black">View Action Plan <ArrowRight size={13} /></Link> : !failed && <button type="button" onClick={onGenerate} disabled={planning} className="mt-4 flex w-full items-center justify-center gap-2 rounded-[7px] border border-[rgb(167_139_250/0.3)] bg-[rgb(167_139_250/0.08)] px-3 py-2.5 text-xs font-medium text-[var(--intelligence)] disabled:opacity-50">{planning && <LoaderCircle size={13} className="animate-spin" />}{planning ? "Generating plan" : "Generate action plan"}</button>}</section></div>;
}
...
export function CommandCenterScreen() {
  ...
  async function handleGeneratePlan() {
    if (!submittedRequest || planning) return;
    setPlanning(true);
    setPlanningError("");
    try {
      await generatePlan(submittedRequest.id);
      setSubmittedRequest((current) => current ? { ...current, status: "PLANNED" } : current);
      setHistory((current) => current.map((item) => item.id === submittedRequest.id ? { ...item, status: "PLANNED" } : item));
    } catch (caught) {
      if (caught instanceof ApiError && caught.status === 401) void refreshSession();
      if (caught instanceof ApiError && caught.code === "UNSUPPORTED_PLANNING_INTENT") {
        setSubmittedRequest((current) => current ? { ...current, status: "FAILED" } : current);
        setPlanningError("Planning unavailable for this request. The deterministic planner supports a limited set of governed operations. Refine the request and try again.");
      } else setPlanningError(caught instanceof ApiError ? caught.message : "Unable to generate an action plan.");
    } finally {
      setPlanning(false);
    }
  }
  ...
  return <AppShell pageTitle="AI Command Center"><main className="mx-auto max-w-[1600px] px-4 py-6 sm:px-6 lg:px-8 lg:py-8"><div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-end"><div><p className="text-[10px] font-medium uppercase tracking-[0.18em] text-[var(--intelligence)]">AI operations / intake</p><h1 className="mt-2 text-3xl font-semibold tracking-[-0.035em]">AI Command Center</h1><p className="mt-2 max-w-2xl text-sm text-[var(--text-secondary)]">Record an operational request as the governed root for future planning and review.</p></div><div className="flex gap-2"><button type="button" onClick={() => void toggleHistory()} className="flex items-center gap-2 rounded-[7px] border border-white/[0.08] px-3 py-2 text-xs text-[var(--text-secondary)] hover:bg-white/[0.04]"><FileClock size={14} />History</button><button type="button" onClick={reset} className="flex items-center gap-2 rounded-[7px] bg-white px-3 py-2 text-xs font-medium text-black hover:bg-zinc-200"><Plus size={14} />New request</button></div></div>
    ...
    {submittedRequest ? <PersistedContext request={submittedRequest} planning={planning} planningError={planningError} onGenerate={() => void handleGeneratePlan()} /> : <PreviewBoundary />}
  </main></AppShell>;
}
