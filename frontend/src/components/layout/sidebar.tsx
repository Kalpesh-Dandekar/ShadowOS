...
import { navigationForRole, roleLabel, userInitials } from "../../lib/navigation";
import type { AuthUser } from "../../types/auth";
import { Brand } from "../ui/brand";

const icons = { "/dashboard": Gauge, "/command-center": TerminalSquare, "/action-plans": ListChecks, "/shadow-workspace": Blocks, "/risk": ShieldAlert, "/policies": BookOpenCheck, "/approvals": ClipboardCheck, "/executions": Bot, "/audit": FileClock, "/analytics": BarChart3, "/activity": Activity, "/notifications": Bell, "/settings": Settings } as const;

type SidebarProps = { user: AuthUser; collapsed: boolean; mobileOpen: boolean; onCollapse: () => void; onCloseMobile: () => void };

export function Sidebar({ user, collapsed, mobileOpen, onCollapse, onCloseMobile }: SidebarProps) {
    ...
    <>
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 flex flex-col border-r border-white/[0.07] bg-[#090b0e] shadow-[12px_0_40px_rgb(0_0_0/0.08)] transition-[width,transform] duration-200 ${collapsed ? "w-[72px]" : "w-[248px]"} ${mobileOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}>
        ...
        <div className="border-t border-white/[0.06] p-3">
          ...
          <div className={`flex items-center ${collapsed ? "justify-center" : "gap-3"}`}><span className="grid size-8 shrink-0 place-items-center rounded-[7px] bg-white/[0.07] text-xs font-semibold">{userInitials(user.name)}</span>{!collapsed && <div className="min-w-0"><p className="truncate text-xs font-medium">{user.name}</p><p className="text-[10px] text-[var(--text-muted)]">{roleLabel(user.role)}</p></div>}</div>
        </div>
