...
export function TopCommandBar({ onMenu, onPalette }: { onMenu: () => void; onPalette: () => void }) {
  return <header className="sticky top-0 z-30 flex h-16 items-center gap-4 border-b border-white/[0.07] bg-[#08090be8] px-4 backdrop-blur-xl sm:px-6">
    ...
    <div className="hidden items-center gap-2 text-xs sm:flex"><span className="text-[var(--text-muted)]">ShadowOS</span><span className="text-[var(--border-strong)]">/</span><span className="text-[var(--text-secondary)]">Governance Overview</span></div>
    <button onClick={onPalette} className="ml-auto flex h-9 w-full max-w-[300px] items-center gap-2 rounded-[7px] border border-white/[0.08] bg-white/[0.025] px-3 text-xs text-[var(--text-muted)] hover:border-white/15 hover:bg-white/[0.04] sm:ml-auto"><Search size={14} /><span className="truncate">Search commands</span><kbd className="ml-auto hidden rounded border border-white/10 bg-white/[0.04] px-1.5 py-0.5 font-mono text-[9px] md:block">⌘ K</kbd></button>
